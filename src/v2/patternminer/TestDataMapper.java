...
public class TestDataMapper {
	...
	public TestDataMapper(SensorUtil the_sensors, final String a_Pattern_mapper_file)
			throws ClassNotFoundException, IOException {
		initialise2LPattern(a_Pattern_mapper_file);
		// printSetup();
		my_sensors = the_sensors;
	}

	private void initialise2LPattern(final String a_Pattern_mapper_file)
			throws IOException, ClassNotFoundException {
		{
			FileInputStream fis = new FileInputStream(a_Pattern_mapper_file);
	...
	public void startMapping(final List<ActivitySensorAssociation> sensorData, final String output_file)
			throws IOException, ClassNotFoundException {
		List<Integer> acts = new ArrayList<Integer>();
		List<String> patterns = new ArrayList<String>();
		for (ActivitySensorAssociation asa : sensorData) {
	...
	public static void run(final int[] acts, final String appendix) throws IOException, ClassNotFoundException {
		...
		for (int i = 0; i < Parameters.NUM_OF_FOLDERS; i++) {
			...
			TestDataMapper pm = new TestDataMapper(getSensors(), group_dir + Parameters.PATTERNS_MAPPER_DIR + appendix + i);
			pm.startMapping(test.get(i), group_dir+Parameters.PATTERNS_SQUENTIAL_CONVERTED_TEST_DIR+appendix+i);
		}
	}
	
	private static SensorUtil getSensors() throws IOException, ClassNotFoundException {
